...
def read():
    ...
    # converters = {}
    # for i in range(0,13):
    #     converters[i] = lambda x: ignoreMissingValues(x)
   
    
    sed = np.genfromtxt('marketing.dat', comments='@',delimiter=',') 
    idx = np.any(np.isnan(sed),axis=1)
    sed = np.delete(sed,idx,axis=0)
    # sed[idx] = []
